...
include_once('config.inc.php');		 // curl header info
...
#-#############################################
# desc: constructor
function __construct($needle, $userAgentString, $chead) {
	$this->needle=$needle;
...
# returns: array if successful, and false if failure
function test_proxies($arrProx,$proxtype,$urltype,$hostname) {
	$externurls = array(
	...
	if(strcmp($urltype,'external')) {	// connect internally
	...
	for($i=0; $i < $numcon; $i++) {	// here be dragons
		...
		for($j=0; $j < MAX_CONNECTIONS; $j++) {
			...
			if(strcmp($urltype,'internal')) {	// connect externally
				...
				if(!strcmp($hostname,'localhost')) {	// they are trying to externally connect to an internal site
					$_SESSION['warnmsg'] = 'You cant connect external proxies to localhost!';
					return false;
				}
				//$myip = $_SERVER['REMOTE_ADDR'];
				if($hostname[strlen($hostname)-1] != "/"){ $hostname .= "/"; }
				$myip = $externip;	// make sure to check for external ip and not localhost
				$url  = $hostname."{$_SERVER['SCRIPT_NAME']}?".PROXY_REQ_VAR;
		...
		for($k=0; $k < count($result); $k++) { // loop through the result set checking for the needle
			if(($result[$k] == FALSE) || (strpos($result[$k],'REMOTE_ADDR') === false)) {	// are we at least getting the page
				...
				#$this->query_update(TBL_GOOD,$column,$where);		
				$yatmpvar = array('ip' => $connectlist[$k][0], 'port' => $connectlist[$k][1], 'type' => $connectlist[$k][2], 'alevel' => 'anonymous');
				$yatmpvaranon[] = $yatmpvar;			// update timestamp
				$cntranon++;
				...
				$yatmpvar = array('ip' => $connectlist[$k][0], 'port' => $connectlist[$k][1], 'type' => $connectlist[$k][2], 'alevel' => 'identifiable');
				$yatmpvarident[] = $yatmpvar;			// update timestamp
				$cntrgood++;
			}
		}
	...
	$arryatmp = array_merge($yatmpvaranon,$yatmpvarident);
	
	if(AUTO_DUPE_REMOVE == 1) {	// make sure there are no duplicates after testing
		for($i=0;$i<count($arryatmp);$i++) {		// duplicate removal
			$hash = md5($arryatmp[$i]['ip'].$arryatmp[$i]['port']); // md5 to the rescue again
			if (!isset($arrayHashes[$hash])) {		// if the md5 didn't come up, keep track of it, otherwise its a dupe
				...
				// php sucks as unset($arryatmp[$i]); does not work all the time in php5 :/
				unset($arryatmp[$i]['ip']);   // unset dupe
				unset($arryatmp[$i]['port']); // unset dupe
				unset($arryatmp[$i]['type']); // unset dupe
			}
		}

		$yatmpar = array();
		for($i=0;$i<count($arryatmp);$i++) {		// duplicate removal
				if(isset($arryatmp[$i]['ip'])) {
					$yatmpar[] = $arryatmp[$i];
				}
		}	
	}
...
function check($arrProx,$proxtype) { // we be checkin
	if($proxtype == 'new') { $newmax = MAX_CONNECTIONS * 2; } else { $newmax = MAX_CONNECTIONS; }	// compensate for duplicates
	$listcount   = count($arrProx); 		       // needed multiple times
	...
	for($i=0; $i < $numcon; $i++) {	// here be dragons 
		...
		$result = $curl->exec();
		if($proxtype == 'new') {	// we need to play games with the counter
			for($k=0; $k < count($result); $k++) {	// loop through the result set checking for the needle
